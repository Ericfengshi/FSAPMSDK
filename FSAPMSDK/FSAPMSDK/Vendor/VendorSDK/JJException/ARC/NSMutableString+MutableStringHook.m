...
+ (void)jj_swizzleNSMutableString{
    ...
    swizzleInstanceMethod(NSClassFromString(@"__NSCFString"), @selector(replaceCharactersInRange:withString:), @selector(hookReplaceCharactersInRange:withString:));
}
...
- (void)hookReplaceCharactersInRange:(NSRange)range withString:(NSString *)str {
    if (str){
        if (range.location + range.length <= self.length) {
            ...
            handleCrashException(JJExceptionGuardNSStringContainer,[NSString stringWithFormat:@"NSMutableString replaceCharactersInRange:withString string:%@ range:%@",str,NSStringFromRange(range)]);
        }
    }else{
        handleCrashException(JJExceptionGuardNSStringContainer,@"NSMutableString replaceCharactersInRange:withString: string nil");
    }
}
